...
#                      **頂点フィルタをかけない**
#   R（集約ルート）    = P のうち頂点であるもの
#
# 収集位置は検査 13 の判定と**同一**でなければならない。片方だけ「戻り値位置のみ」に
# すると、書き込み専用ストアの型が P に入らず検査 14 / 15 の対象から外れる。
#
# **P と R を分けてあるのは検査 14 のためである。** 検査 14 に R を渡すと
# **証明可能に発火しなくなる**: ルート B がルート A のフィールドになった瞬間、B は
# 「含まれる型」になり、定義により頂点でなくなって R から脱落する。ところが
# 「B が A に含まれる」は検査 14 が検出したい条件そのものなので、R を使うと
# 検出条件と対象条件が排他になり、検査 14 は比較する組を失って沈黙する。
#
# P なら「OrderStore が Order を運んでいる」という事実は B が非頂点になっても変わらない
# ので、(A=Shipment, B=Order) の組が作れて発火する。健全なツリーでは P = R であり、
# green のときの挙動は変わらない。
...
# P を返す（集約ストアポートが運ぶドメイン型。頂点フィルタなし）。
aggregate_store_types() {
  port_domain_types "$1" | awk '$2 != "!NESTED" { print $2 }' | sort -u
}

# R を返す（P のうち頂点であるものだけ）。
aggregate_roots() {
  ...
  while IFS= read -r t; do
    ...
    printf '%s\n' "$t"
  done < <(aggregate_store_types "$ports")
}
...
#
# 判定: **P**（集約ストアポートが運ぶ型）の各要素 A について、他の要素 B が
#   (a) `type A struct { … }` のフィールド型に現れる
#   (b) `func (x *A)` / `func (x A)` で始まるメソッドのシグネチャ（引数・戻り値）に現れる
# のどちらかで語境界一致したら fail。
#
# **R ではなく P を使う。** R（頂点であるものだけ）を使うと (a) が証明可能に発火しない —
# B が A のフィールドになった瞬間に B が頂点でなくなって R から落ちるためである
# （理由は上の定義ブロック）。健全なツリーでは P = R なので green の挙動は変わらない。
#
# **識別子による参照は自動的に許される。** Shipment.orderID OrderID の OrderID は
# 集約ストアポートが運ぶ型ではないので判定の対象に入らず、加えて語境界規約により
# OrderID が Order に前方一致することもない。この 2 段構えが要る —
# 意味論だけでは字面の誤検出を防げない。
#
