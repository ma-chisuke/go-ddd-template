...
- **規約は 2 文書**。識別子・ファイル・パッケージ・言語ポリシーは `CONVENTIONS.md`（A / B / F / G 群）、
  テスト関数名・サブテスト名・テストの日本語コメントは `docs/testing-conventions.md`（C / D / E 群）。
  サブテスト名（`t.Run` とテーブル駆動の `name`）は 8 語の閉じた語彙
  （`正常系` `異常系` `境界` `冪等` `並行` `契約` `回帰` `性質`）+ `: ` で始め `/` を含めない。
  強制は `make lint` と `make conventions`（どちらも `make ci` に含まれる）。
