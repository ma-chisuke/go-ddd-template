...
例示テストは「この入力ならこの結果」を固定します。可換律のような**代数的法則**や、
「任意の操作列でこの不変条件が保たれる」という主張は、例示をいくら並べても言い尽くせません。
そこだけを**生成器が入力を振るテスト**が受け持ちます。

道具は 2 つあり、**受け持つ主張が違うので一方に寄せません**。

### H-1 Go ネイティブ fuzz は境界の全域性に使う

`Fuzz<主題>(f *testing.F)` は依存ゼロで書けます。受信アダプタの入口に対して
「任意の入力で壊れない」ことを主張する用途に使います。seed corpus は通常のテスト実行で走る
ので（`Fuzz<主題>/seed#0` のようなサブテストとして現れます）、マージゲートの回帰になり、
カバレッジにも算入されます。

単位は**受信サーバ 1 つにつき 1 ターゲット**です。応答の語彙も種別 URI の名前空間も
コンテキストが所有しているので、複数のコンテキストを 1 本にまとめると、どちらの契約を
検証しているのかが曖昧になります。

> **fuzz は `httptest` サーバを経由せずハンドラを直接呼びます。** `net/http` のサーバは
> ハンドラの panic を**回復してログへ落とす**ため、サーバ越しでは panic がテストの失敗として
> 現れず、「panic しない」という主張が成立しません。テスト用の結線ヘルパーは
> `newHandler`（`http.Handler` を返す）と `newServer`（それを `httptest` で起こす）に
> 分けてあり、fuzz は前者を使います。

### H-2 `pgregory.net/rapid` はドメインの不変条件に使う

`rapid`（テスト専用依存）は生成器と**縮約**（反例を最小形へ削る）を持ちます。値オブジェクトの
往復・代数的法則と、集約の状態機械に使います。

状態機械は `rapid.Check` と `(*rapid.T).Repeat` で書きます（`rapid.Run` という関数は
**ありません**）。不変条件は `actions[""]` に置きます — ライブラリの doc が
「executed before/after every other action invocation and should only contain invariant
checking code」と定めており、保存則や非負のような「毎回成り立つべきこと」がまさにこれです。

```go
rapid.Check(t, func(t *rapid.T) {
    item := newSeededItem(t, 0)
    t.Repeat(map[string]func(*rapid.T){
        "":        func(t *rapid.T) { /* 不変条件だけを書く */ },
        "reserve": func(t *rapid.T) { /* … */ },
    })
})
```

> **生成器が空転していないかを疑ってください。** 状態機械は書いた瞬間から緑になるので、
> 「検査しているつもりで一度も踏んでいない経路」に自力では気づけません。実例が 2 つあります。
>
> - **冪等性**は、同じ参照を 2 回以上使わなければ一度も検証されません。毎回新しい ID を
>   生成する素朴な生成器では空転します。参照は**小さな固定集合**から引きます。
> - **前提を満たさない呼び出しの拒否**（在庫不足など）は、「前提を満たさなければ `t.Skip`」を
>   一律に当てると毎回飛ばされて空転します。**拒否を主張したい操作だけは skip しません。**
>
> 疑わしいときは、前提を**構成して必ず踏む**専用の性質テストを別に置いてください。
> 状態機械の探索まかせでは「たまたま一度も踏まなかった」可能性を排除できません。
> 加えて、**壊した実装を一時的に注入して当該テストが落ちることを確認**してください
> （[../CONVENTIONS.md](../CONVENTIONS.md) の G-1 と同じ考え方を、lint rule ではなく
> テストに当てたものです）。

生成域は**オーバーフローしない範囲**に絞ります。主張したいのは代数的法則であって整数型の
限界ではないので、振り切って「ドメインのバグではない失敗」を報告させても意味がありません。

時刻を含む集約では、**メソッドの署名だけを見て「時刻は引数だからクロック注入は不要」と
決めないでください。** 引数で `now` を受け取るメソッドの隣で、別のメソッドが内部で
`time.Now()` を呼んで期限を起算していることがあります（この 2 つが噛み合っていないと、
固定基準から作った `now` では期限切れの経路に一度も入りません）。基準時刻を 1 度だけ取り、
判定の境界がテストの実行時間より桁違いに大きくなる幅で振ってください。

### H-3 `testing/quick` は使わない

標準ライブラリですが doc に "The testing/quick package is frozen and is not accepting new
features." と明記されています。

### H-4 命名とファイル

関数名は `Test<主題>_<性質>`（`TestMoney_AddIsCommutative` / `TestSKU_RoundTrips`）と
`Fuzz<主題>` です。ファイルは `<主題>_property_test.go` / `<主題>_fuzz_test.go` にし、
対応する `<主題>.go` の隣に置きます。

### H-5 `t.Run` の分類語彙は `性質:`

D-2 の 8 語に含まれています。**生成器で入力を振っている時点で `性質` に確定します** —
D-2 の判定は上から順に評価して最初に該当したものを採り、`性質` が 1 番目だからです。
「数量 0 は `ErrInvalidQuantity`」のように単体なら `境界` になるケースでも、生成器で
振っているなら `性質` です。

### H-6 マージゲートに載せるのは既定回数・既定シードのぶんだけ

`rapid.Check`（既定回数・既定シード）と fuzz の seed corpus だけがゲートに載ります。
**長時間の `-fuzz` は載せません** — 打ち切り時刻でしか止まらないので、同じコミットでも
実行のたびに結果が変わりえます。ゲートに載せると赤の意味が「壊れた」から
「今日は運が悪かった」に薄まります。探索は任意実行の `make fuzz` で行います
（1 ターゲットあたりの時間は `make fuzz FUZZTIME=5m` で伸ばせます）。

### H-7 反例の回帰化は非対称（意図的）

| 道具 | 反例ファイル | 扱い |
| --- | --- | --- |
| fuzz | `testdata/fuzz/Fuzz<主題>/` | **コミットする**（Go 公式のワークフロー。通常のテスト実行で毎回走る回帰になります） |
| `rapid` | `testdata/rapid/*.fail` | **`.gitignore` する**（タイムスタンプと pid 付きのスクラッチで、実行のたびに別名で積み上がります） |

`rapid` が見つけた反例は、縮約された形を `t.Run("回帰: …")` の**決定的なケース**として
テストに書き下します。ファイルではなくコードが回帰の記録になります。
...
| H-4 `<主題>_property_test.go` / `<主題>_fuzz_test.go`（カタログ的な語幹の禁止） | `scripts/convention-gate.sh` 検査 4 | fail |
| H-5 `性質:` の語彙 | `scripts/convention-gate.sh` 検査 2 / 2' | fail |
| H-7 `testdata/rapid/` を追跡しない | `.gitignore` | — |
| C-1 / C-2 の形、C-4b、D-3、D-5、E-2〜E-6、H-1〜H-3、H-6 | **人間のレビュー観点**（機械強制なし） | — |
