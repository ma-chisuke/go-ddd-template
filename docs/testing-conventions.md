...
テーブル駆動にするときは**必ず `name` という名前のフィールドを持つ構造体**にし、ケースは
**フィールド名つきで書きます**（位置で並べない）。
→ `scripts/convention-gate.sh`（検査 2' / 3' と検査 8）
...
位置で並べたケース（`{"空 SKU", …}`）は**検査 8 が禁じます**。禁じる理由は、検査 2' が `name:` に
続く文字列を拾う実装なので、位置指定のケース名は構造体に `name` フィールドがあっても**見えない**
からです。禁じておかないと「検査 2' が 0 件」を、準拠しているからなのか見えていないからなのか
区別できません（この盲点は実際に 5 表 25 ケースを見落としていました）。

> **既知の限界（検査 8）**: 検出は「行頭が `{`」を手がかりにしており、gofmt が 1 行 1 要素に
> 整形することを前提にしています。`[]T{{…}, {…}}` のように 1 行へ詰め込んだ形は見えません。
> 対象は `*_test.go` だけです（本番コードの unkeyed リテラルは `go vet` の `composites` が
> import した型について別に見ています）。テーブル以外の正当な位置指定がどうしても必要になったら、
> `scripts/convention-gate.sh` 冒頭の `POSITIONAL_EXCLUDE_FILES` に理由つきで列挙してください。

> **既知の限界（検査 2 / 2'）**: `t.Run(fmt.Sprintf(…), …)` のように**サブテスト名を実行時に
> 組み立てる場合、機械検査できません**（名前が静的に存在しないため）。この形を使うときは、
> 書式文字列そのものに分類を織り込んでください。
>
> ```go
...
| D-6 位置指定で書かない（検査 2' の視界の担保） | `scripts/convention-gate.sh` 検査 8 | fail |
