...
| --- | --- | --- |
| 集約（Aggregate） | 同時に一貫していなければならないオブジェクトの塊。外からは集約ルート経由でのみ操作する | `Order`（[../contexts/ordering/internal/domain/order/order.go](../contexts/ordering/internal/domain/order/order.go)）／ `StockItem`（[../contexts/inventory/internal/domain/inventory/stock_item.go](../contexts/inventory/internal/domain/inventory/stock_item.go)） |
| エンティティ（Entity） | 同一性を持ち、状態が時間とともに変わるもの | `Reservation`（[../contexts/inventory/internal/domain/inventory/reservation.go](../contexts/inventory/internal/domain/inventory/reservation.go)）— 集約 `StockItem` の子 |
| 値オブジェクト（Value Object） | 同一性を持たず、値そのもので等価性が決まる不変の型 | `SKU` / `Quantity` / `Money` / `ReservationRef` / `CustomerID` / `OrderID`。**境界ごとに独立所有**する（[../contexts/ordering/internal/domain/order/](../contexts/ordering/internal/domain/order/) と [../contexts/inventory/internal/domain/inventory/](../contexts/inventory/internal/domain/inventory/) に同名で別の型がある） |
| ドメインイベント（Domain Event） | ドメインで起きた「事実」。過去形で名づける | 注文 2 種（[../contexts/ordering/internal/domain/order/event.go](../contexts/ordering/internal/domain/order/event.go)）と在庫 5 種（[../contexts/inventory/internal/domain/inventory/event.go](../contexts/inventory/internal/domain/inventory/event.go)）の計 7 種。集約が記録し、`PullEvents` で取り出す |
| ドメインサービス（Domain Service） | 1 つの集約に閉じない振る舞い。状態を持たない | `ReservationService`（[../contexts/inventory/internal/domain/inventory/reservation_service.go](../contexts/inventory/internal/domain/inventory/reservation_service.go)）— マルチ SKU 予約の全か無か。**リポジトリを引かず**、ユースケースから引き当て済みの集約を受け取る |
...
| 再構成（Reconstitution） | 永続化された状態から集約を組み立て直す。検証済みなのでイベントは発生させない | `ReconstituteOrder`（[../contexts/ordering/internal/domain/order/order.go](../contexts/ordering/internal/domain/order/order.go)）／ `ReconstituteStockItem`・`ReconstituteReservation`（[../contexts/inventory/internal/domain/inventory/stock_item.go](../contexts/inventory/internal/domain/inventory/stock_item.go)） |
...
| 腐敗防止層（Anti-Corruption Layer） | 相手のモデルを自分のモデルへ翻訳し、相手の概念が漏れ込むのを防ぐ層 | [../contexts/ordering/internal/adapter/outbound/aclhttp/](../contexts/ordering/internal/adapter/outbound/aclhttp/) — ポートは [../contexts/ordering/internal/application/acl.go](../contexts/ordering/internal/application/acl.go)。在庫の 409 / 5xx を注文側の番兵へ翻訳する |
