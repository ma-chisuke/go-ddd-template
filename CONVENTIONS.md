...
### B-10 集約ルートとリポジトリは 1 対 1 に対応する

> **(a)** `application.Repos` のアクセサが返すストアポートのメソッドシグネチャに
> **`*domain.T` / `[]*domain.T` の形で**現れる型は、すべて `AggregateRoot` を満たさなければ
> ならない。
> **(b)** `AggregateRoot` を満たす型は、すべて `Repos` のいずれかのストアポートに
> **`*domain.T` / `[]*domain.T` として**扱われていなければならない。

→ `scripts/convention-gate.sh`（検査 13）

**両方を課して初めて 1 対 1 が固定されます。** (a) だけなら「子にリポジトリを与える」違反しか
捕まらず、(b) だけなら「集約ルートを永続化しない」違反しか捕まりません。

**ポインタの形への限定は必須です。** 限定しないと現ツリーで即座に偽陽性が出ます — ストア
ポートは値オブジェクトを**値で**引数に取るからです（`OrderStore.Load(ctx, id domain.OrderID)` /
`StockStore.LoadMany(ctx, skus []domain.SKU)`）。集約ルートは必ずポインタで受け渡され、
値オブジェクトは値で渡ります。これは B-9（検査 12）と表裏で、両者を合わせると
**「ポインタで現れる `domain` 型は集約ルートだけ」**という 1 つの性質になります。

**`Outbox()` に除外リストは設けません。** `MessagePublisher` は `outbox.Message` だけを扱い
`domain` の型を一切扱わないため、(a) の条件に自然に該当しません。名前で除外すると、
名前を変えた瞬間に検査が緩みます。

`ports.go` だけを見れば済むのは、B-5 の双方向の約束（検査 10 / 11）がポート宣言の全数性を
保証しているからです。この検査はその保証の上に成り立っています。

**`Repos` にアクセサを増やす理由を取り違えないでください。** `Shipments()` が在るのは
「`Shipment` の書き込みとアウトボックス投入を同一トランザクションで行うため」であって、
「`Order` と `Shipment` を一緒に書くため」ではありません。出荷のユースケースは `Orders()` を
使わず、トランザクションの内側で触れる集約ルートは 1 つだけです
（1 トランザクション 1 集約という指針は保たれています）。
...
| 20 | 集約ルートとリポジトリの 1 対 1 対応（B-10。(a) 子にリポジトリを与えない／(b) 集約ルートを永続化し忘れない） | `scripts/convention-gate.sh` 検査 13 | **fail** |
| 21 | 集約ストア実装のファイル名（B-11） | `scripts/convention-gate.sh` 検査 14 | **fail** |
...
計測は残します — 検査 12 / 13 / 14 の追加による `make conventions` の増加は **約 +0.7 秒**
（同一マシンで 3 回計測: 検査 11 本 3.43 / 3.45 / 3.46 秒 → 検査 14 本 4.11 / 4.23 / 4.10 秒）。
増加分の大半は検査 12 が各ドメインファイルを awk で 1 回走査する分です。
