...
集約とは**包含の木**であり、集約ルートとはその**頂点**です。この節の規則は、その木の形を
コードの字面から導けるように保ち、壊れたときに CI が止まる状態を作るためにあります。

**集約ルートを宣言するマーカー型もインタフェースもありません。** 頂点かどうかは「他の
ドメイン型のフィールドとして現れるか」で決まり、リポジトリを持つ型かどうかは `ports.go` の
ポートのシグネチャで決まります。どちらも既にコードにある事実なので、判定のためだけの
構築物を足していません。

> **この節の規則は Go の一般規則ではありません。** 「内側の型を値で渡す」も「集約間は
> 識別子で参照する」も Go コミュニティの慣習ではなく、**DDD の集約境界を機械強制する
> ために本テンプレートが選んだ規則**です。Go 由来の強制規則は「命名」と
> 「Go 由来の実装作法」の節にあります。

### R-1 内側の型はポインタとして集約の外へ出さない

他のドメイン型のフィールドとして含まれる型（`OrderLine`・`Reservation`・値オブジェクト）は、
`*T` の形でドメインパッケージに現れません。値で渡します。**検査 12 が機械強制します。**

根拠は「集約の内側の状態を変更できるのは、その型が属する集約のルートだけ」という不変条件
です。ポインタを外へ渡すと、受け取った側が集約を経由せずに内側の状態を書き換えられます。

### R-2 集約ルートは他の集約ルートを保持・受け渡ししない

集約ルートは、別の集約ルートをフィールドにも自分のメソッドのシグネチャにも持ちません。
**集約間は識別子で参照します**（`Shipment` は `Order` ではなく `OrderID` を持つ）。
**検査 14 が機械強制します。**

1 コンテキストのドメインパッケージは 1 つなので（B-8）、2 つの集約ルートは必ず同じ
パッケージに同居します。Go の可視性はパッケージ単位で型単位ではないため、R-1 だけでは
同一パッケージ内の集約間の漏洩を防げません。

### R-2b 内側のコレクションを返すアクセサは複製を返す

スライスとマップは参照型なので、`return o.lines` と書くとポインタ無しで背後を共有します。
`Order.Lines()` と `StockItem.Reservations()` は複製を返します。**これは機械検査していません**
（下の「機械検査していない範囲」の #2）。レビューで守ります。

### R-3 リポジトリを持てるのは集約の頂点だけ

`ports.go` のポートが**戻り値位置またはポインタ引数位置**で運ぶドメイン型は、他のドメイン型に
含まれていてはなりません。**検査 13 が機械強制します。**

- **戻り値位置**はポインタ・値を問いません。「値で書けば検査をすり抜ける」を塞ぐためです
- **引数位置はポインタの形だけ**を見ます。索引キーは値で渡り（`Load(ctx, id domain.OrderID)`）、
  集約はポインタで渡る（`Save(ctx, o *domain.Order)`）という一貫した形がそのまま判別子になります
- `Repos` に載らないポート（`StockReserver` / `EventDispatcher` / `Clock`）も走査します。
  `Repos` 経由だけを見ると、集約ストアを `Repos` の外へ置くだけで検査を回避できてしまいます

### 集約ストアポート・`P`・`R`（**検査 13 / 14 / 15 が参照する唯一の定義**）

3 本の検査は同じ導出を共有します。**定義はここ 1 箇所だけです** ——
`scripts/convention-gate.sh` はこの節を参照し、独自の定義を持ちません。

| 記号 | 定義 |
| --- | --- |
| 集約ストアポート | `ports.go` のポートのうち、**戻り値位置またはポインタ引数位置**にドメイン型を運ぶもの |
| `P` | 集約ストアポートが上記 2 つの位置で運ぶドメイン型。**頂点フィルタをかけません** |
| `R` | `P` のうち**頂点である**もの（＝集約ルート） |

収集位置は R-3 の判定と**同一**でなければなりません。片方だけ「戻り値位置のみ」にすると、
書き込み専用ストアの型が `P` に入らず検査 14 / 15 の対象から外れます。

現ツリーでの実測は `P` = `R` です（`ordering` は `Order` と `Shipment`、`inventory` は
`StockItem`）。`MessagePublisher` / `Repos` / `StockReserver` / `EventDispatcher` / `Clock` は
ドメイン型を運ばないので、**除外リスト無しで自然に外れます**。

> **検査 14 が `R` ではなく `P` を使う理由。** `R` を使うと (a)（フィールド）が
> **証明可能に発火しません** —— ルート `B` がルート `A` のフィールドになった瞬間、`B` は
> 「含まれる型」になって頂点でなくなり `R` から脱落しますが、「`B` が `A` に含まれる」は
> 検査 14 が検出したい条件そのものだからです。検出条件と対象条件が排他になります。
> `P` なら「`OrderStore` が `Order` を運んでいる」という事実は `B` が非頂点になっても
> 変わらないので、組を作れて発火します。`P` = `R` の健全なツリーでは挙動は同じです。

### R-4 / R-5 集約ストアの名前と置き場所

- **R-4**: 集約ストアの実装は `<x>_store.go` に置きます。**検査 15 が機械強制します。**
- **R-5**: `Store` の語は**集約ストアのポートとその実装にのみ**使います。集約の backing store は
  `<X>Rows` と名づけます（`OrderRows` / `StockRows` / `ShipmentRows`）。

**`<X>` はポート名の語幹であり、集約ルートの型名ではありません。** `StockStore` は `StockItem`
集約のストアなので `<X>` は `Stock` です。したがって正しい名前は `stock_store.go` / `StockRows`
であって、`stock_item_store.go` / `StockItemRows` ではありません。

アウトボックスの backing store（`Stores` / `NewStores()`）と `postgres.outboxStore` は
**この規則の主語の外**です。主語は「**集約の** backing store」であり、`MessagePublisher` は
ドメイン型を運ばないので集約ストアポートではありません（語幹 `outbox` がそもそも導出されず、
除外リスト無しで外れます）。

### 機械検査していない範囲（**6 項目**）

R-1 が担保するのは「**ポインタ経由の**漏洩が不可能」までであって、「漏洩が不可能」では
ありません。次は**機械が止めません**。担保できたと感じた側にこそ穴が残るので、名指しで挙げます。

| # | 範囲 | 実例 |
| --- | --- | --- |
| 1 | 頂点の公開フィールドが、含まれる型を晒している | `FieldViolation.Rule` は公開で `Rule.Field` / `Code` / `Err` も公開。`*FieldViolation` は正当に現れるので、パッケージ外から `v.Rule.Code = "…"` と書けます |
| 2 | コレクションを複製せずに返すアクセサ（R-2b） | 現状 `Order.Lines()` と `StockItem.Reservations()` は複製していますが、**複製したことを検査していません** |
| 3 | ドメインサービスが複数のルートを引数で受け取る経路 | `ReservationService.Allocate(items []*StockItem, …)`。複数集約に跨る調整はドメインサービスの正当な責務なので禁じられません |
| 4 | ドメインパッケージのフリー関数が複数のルートを受け取る場合 | 3 と同じ形。ルートのメソッドではないので検査 14 の主語に入りません |
| 5 | メソッド内でローカルに構築した他集約の値への代入 | 誰の状態でもない値の変更で実害がなく、字面では区別できません |
| 6 | 集約を**値**で受け取り、戻り値にドメイン型を持たない書き込み専用ポート | 検査 13 の 2 つの収集位置のどちらにも現れません。**書けば楽観的排他制御が壊れます**が（`MarkPersisted` の結果が呼び出し元へ戻らない）、機械が止めるわけではありません |

> **3 と 4 の残余は「公開メソッド経由に限られる」ではありません。** 同一パッケージでは
> ルートの非公開フィールドへの直接代入も非公開メソッドの呼び出しも合法です
> （実例: `reservation_service.go` は `stock_item.go` の非公開メソッド `hasReservation` を呼びます）。

### 機械検査していないのではなく、他の検査に依存している前提

次は**上の一覧とは別枠**です。機械は止めますが、検査 13 単独の保証ではありません。

| 前提 | 依存先 |
| --- | --- |
| ポート宣言が `ports.go` の外に置かれないこと | **検査 10**。検査 13 は `ports.go` を全走査するので、検査 10 が守られている限り「ポートでありながら視界の外」は生じません |

集約ルートを 1 つ足す手順は [docs/add-an-aggregate.md](docs/add-an-aggregate.md) にあります。
...
計測は残します —— 集約境界の 4 本（#19〜#22 / 検査 12〜15）を足したときの `make conventions` は
**3.16 秒 → 4.87 秒**（各 3 回の中央値。macOS / Apple Silicon）で、**約 +1.7 秒**でした。
支配的なのは包含判定をドメイン型ごとに呼ぶ形（型数 × `awk` の起動）です。
