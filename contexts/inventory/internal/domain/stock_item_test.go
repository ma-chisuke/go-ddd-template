...
func TestNewStockItem(t *testing.T) {
	...
	t.Run("異常系: 空 id は不正", func(t *testing.T) {
		...
		_, err := domain.NewStockItem("", mustSKU(t, "WIDGET-001"))
...
func TestStockItem_Replenish(t *testing.T) {
	...
	t.Run("正常系: 利用可能在庫が増え、イベントが記録される", func(t *testing.T) {
		...
		item, _ := domain.NewStockItem("id-1", mustSKU(t, "WIDGET-001"))
		...
		first, ok := events[0].(domain.StockReplenished)
	...
	t.Run("異常系: 補充数量 0 は ErrInvalidQuantity", func(t *testing.T) {
		...
		item, _ := domain.NewStockItem("id-1", mustSKU(t, "WIDGET-001"))
		require.ErrorIs(t, item.Replenish(mustQuantity(t, 0)), domain.ErrInvalidQuantity)
		// 失敗時はイベントも記録されない。
...
func TestReconstituteAndMarkPersisted(t *testing.T) {
	...
	item := domain.ReconstituteStockItem("id-9", mustSKU(t, "GADGET-9"), mustQuantity(t, 42), 3, nil)
...
func TestNewSKU(t *testing.T) {
	...
	t.Run("異常系: 空文字は ErrInvalidSKU", func(t *testing.T) {
		...
		for _, in := range []string{"", "   ", "\t"} {
			_, err := domain.NewSKU(in)
			require.ErrorIs(t, err, domain.ErrInvalidSKU, "NewSKU(%q)", in)
		}
	})
}
