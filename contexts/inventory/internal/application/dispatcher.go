...
//
// ドメインの inventory.DomainEvent は EventName() を持つため、shared/event.Event を
...
func (d *InProcessDispatcher) Dispatch(ctx context.Context, events ...inventory.DomainEvent) {
	...
	// inventory.DomainEvent は shared/event.Event を構造的に満たすため、そのまま適合できる。
