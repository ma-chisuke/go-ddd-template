...
func reconstituteShipment(row sqlcgen.GetShipmentByIDRow) (*domain.Shipment, error) {
	...
	// preparing の追跡番号は空文字で保存されているため、NewTrackingNumber には通さない
	// （空を拒否する検証は「利用者が空を指定した」場合のためのものであり、
	// 未設定という正当な状態を復元するのに使うと復元が失敗する）。
	var tracking domain.TrackingNumber
	if row.TrackingNumber != "" {
		tracking, err = domain.NewTrackingNumber(row.TrackingNumber)
		if err != nil {
			return nil, fmt.Errorf("永続化された追跡番号が不正です: %w", err)
		}
	}
	return domain.ReconstituteShipment(shipmentID, orderID, status, tracking, int(row.Version)), nil
}
...
// update は楽観的排他制御つきで出荷の状態と追跡番号を更新する。
