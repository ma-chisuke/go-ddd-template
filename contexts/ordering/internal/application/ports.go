...
// 書き込みが構造的に起こり得ない。注文ストア・出荷ストアとアウトボックスを、
// 同一トランザクションに束ねて提供する。
//
// 集約ストアが 2 つ並ぶが、1 つのトランザクションで書き込む集約ルートは 1 つに保つ
// （PrepareShipment は注文をトランザクションの外で読み、内では出荷だけを書く）。
// 束ねているのは「同じトランザクションから取り出せる」ことであって、
// 「同じトランザクションで両方を書いてよい」ことではない。
